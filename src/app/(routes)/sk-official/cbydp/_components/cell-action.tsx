...
const CellAction = ({ data }: { data: CBYDPProps }) => {
  ...
  const onArchive = async () => {
    try {
      ...
      toast.success("CBYDP report archived successfully! 🗑️");
      router.refresh();
    } catch (error) {
      toast.error("Failed to archive CBYDP report. 😥");
  ...
  const onRetrieve = async () => {
    try {
      ...
      toast.success("CBYDP report retrieved successfully! 📂");
      setOpenRetrieve(false);
      router.refresh();
    } catch (error) {
      toast.error("Failed to retrieve CBYDP report. 😥");
  ...
  return (
    <>
      <AlertModal
        ...
        onConfirm={onArchive}
        title="Archive CBYDP Report"
        description="Are you sure you want to archive this CBYDP report? This action cannot be undone."
      />
      <AlertModal
        isOpen={openRetrieve}
        onClose={() => setOpenRetrieve(false)}
        onConfirm={onRetrieve}
        title="Retrieve CBYDP Report"
        description="Are you sure you want to retrieve this CBYDP report? It will no longer be archived."
      />
