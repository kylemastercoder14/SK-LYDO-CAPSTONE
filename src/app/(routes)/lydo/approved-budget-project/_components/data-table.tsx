    ...
    <div>
      <div className="flex items-center py-4">
        <div className="flex items-center gap-2">
          <Input
            placeholder="Filter project ID or project name"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-[300px] rounded-none mr-3"
          />
          {table.getColumn("barangay") && (
            <DataTableFilter
              column={table.getColumn("barangay") as Column<TData, unknown>}
              title="Filter Barangay"
              options={BARANGAYS.map((barangay) => ({
                value: normalizeBarangay(barangay),
                label: barangay,
              }))}
            />
          )}
