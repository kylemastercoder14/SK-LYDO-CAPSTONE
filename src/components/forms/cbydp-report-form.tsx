...
import { CBYDP } from "@prisma/client";
...
import { cbydpReportSchema } from "@/validators";
...
const CBYDPReportForm: React.FC<CBYDPReportFormProps> = ({
  initialData,
  onClose,
  userId,
}) => {
  ...
  const form = useForm<CBYDPReportFormValues>({
    resolver: zodResolver(cbydpReportSchema),
  ...
  const onSubmit = async (data: CBYDPReportFormValues) => {
    try {
      setLoading(true);

      if (initialData) {
        await updateCBYDPReport(initialData.id, data, userId);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="pb-4">
          <div className="grid grid-cols-1 gap-x-4 gap-y-5">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>CBYDP Name</FormLabel>
                  <FormControl>
                    <Input
                      disabled={loading}
                      placeholder="e.g., Q2 CBYDP Report"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
...
export default CBYDPReportForm;
