...
import { ProjectReports } from "@prisma/client";
...
import { projectReportSchema } from "@/validators";
...
const ProjectReportForm: React.FC<ProjectReportFormProps> = ({
  ...
  const onSubmit = async (data: ProjectReportFormValues) => {
    try {
      setLoading(true);

      if (initialData) {
        await updateProjectReport(initialData.id, data, userId);
        toast.success("Project report updated successfully! 🎉");
      } else {
        await createProjectReport(data, userId);
  ...
  const title = initialData ? "Edit Project Report" : "Create Project Report";
